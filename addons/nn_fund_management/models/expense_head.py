...
class FundExpenseHead(models.Model):
    ...
    code = fields.Char(string='Expense Head Code', required=True)
    ...
    incoming_transfer_ids = fields.One2many('fund.transfer', 'dest_expense_head_id', string='Incoming Transfers Link')
    outgoing_transfer_ids = fields.One2many('fund.transfer', 'src_expense_head_id', string='Outgoing Transfers Link')
    ...
    _sql_constraints = [
        ('code_uniq', 'unique(code, company_id)', 'The expense head code must be unique per company!')
    ]

    @api.constrains('available_fund')
    def _check_negative_balance(self):
        for head in self:
            if head.available_fund < 0:
                raise ValidationError(_("Negative balance is not allowed. Expense Head '%s' would have a negative available fund of %s.") % (head.name, head.available_fund))
    ...
    def _compute_balances(self):
        for head in self:
            # 1. Total Allocated
            total_allocated = sum(head.allocation_ids.filtered(lambda a: a.state == 'approved').mapped('amount'))
            
            # 2. Incoming Transfers
            incoming_transfers = sum(head.incoming_transfer_ids.filtered(lambda t: t.state == 'approved').mapped('amount'))
            
            # 3. Outgoing Transfers
            outgoing_transfers = sum(head.outgoing_transfer_ids.filtered(lambda t: t.state == 'approved').mapped('amount'))
            
            # 4. Transfer Hold
            transfer_hold = sum(head.outgoing_transfer_ids.filtered(lambda t: t.state in ['submitted', 'gm_approved']).mapped('amount'))
            
            # 5. Requisition Hold
            requisition_hold = sum(head.requisition_ids.filtered(lambda r: r.state in ['submitted', 'gm_approved']).mapped('amount'))
            
            # 6. Spent Amount
            total_spent_amount = sum(head.requisition_ids.mapped('spent_amount'))
            
            # 7. Approved but Unspent
            approved_unspent = sum(head.requisition_ids.filtered(lambda r: r.state == 'approved').mapped('remaining_billable_amount'))
    ...
    def name_get(self):
        result = []
        for head in self:
            name = f"[{head.code}] {head.name}"
            result.append((head.id, name))
        return result
